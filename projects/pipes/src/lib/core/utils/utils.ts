...
export function resolveObject(object: object, keys: string): any {
  let pathArr: string[] = keys.split(".");
  return pathArr.reduce((obj: any, key: string) => (obj && obj[key] !== 'undefined')
    ? obj[key]
    : undefined, object);
}
...
export function takeWhile(input: any[], predicate: CollectionPredicate) {
  return takeUntil(input, (item: any, index: number | undefined, collection: any[] | undefined) => !predicate(item, index, collection));
}
